...
void InotifyEvent::dump_types(std::string &rStr) const {
...
void InotifyWatch::SetMask(uint32_t uMask) noexcept(false) {
  ...
  if (m_wd != -1) {
    int wd =
        inotify_add_watch(m_pInotify->GetDescriptor(), m_path.c_str(), uMask);
    if (wd != m_wd) {
...
void InotifyWatch::SetEnabled(bool fEnabled) noexcept(false) {
  ...
  if (m_pInotify != NULL) {
    if (fEnabled) {
      m_wd = inotify_add_watch(m_pInotify->GetDescriptor(), m_path.c_str(),
                               m_uMask);
      if (m_wd == -1) {
        IN_WRITE_END_NOTHROW
        throw InotifyException(IN_EXC_MSG("enabling watch failed"), errno,
                               this);
      }
      ...
      if (inotify_rm_watch(m_pInotify->GetDescriptor(), m_wd) != 0) {
        IN_WRITE_END_NOTHROW
        throw InotifyException(IN_EXC_MSG("disabling watch failed"), errno,
                               this);
      }
...
void InotifyWatch::__Disable() {
  ...
  if (!m_fEnabled) {
    IN_WRITE_END_NOTHROW
    throw InotifyException(IN_EXC_MSG("event cannot occur on disabled watch"),
                           EINVAL, this);
  }
...
void Inotify::Add(InotifyWatch *pWatch) noexcept(false) {
  ...
  if (pWatch->IsEnabled()) {
    ...
    int wd =
        inotify_add_watch(m_fd, pWatch->GetPath().c_str(), pWatch->GetMask());
    ...
    if (pW != NULL) {
      ...
      if (inotify_add_watch(m_fd, pW->GetPath().c_str(), pW->GetMask()) < 0) {
        ...
        throw InotifyException(
            IN_EXC_MSG("watch collision detected and recovery failed"), errno,
            this);
      } else {
        // recovery failed - go away
        IN_WRITE_END_NOTHROW
        throw InotifyException(
            IN_EXC_MSG("path already watched (but defined another way)"), EBUSY,
            this);
      }
    }
...
void Inotify::wait_for_events(bool fNoIntr) noexcept(false) {
  ssize_t len = 0;
...
bool Inotify::get_event(InotifyEvent *pEvt) noexcept(false) {
  if (pEvt == NULL)
