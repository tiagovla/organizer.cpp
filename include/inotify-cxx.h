...
class InotifyException {
  public:
     ...
     */
    InotifyException(const std::string &rMsg = "", int iErr = 0, void *pSrc = NULL) : m_msg(rMsg), m_err(iErr) { m_pSrc = pSrc; }
...
class InotifyEvent {
  public:
     ...
     */
    InotifyEvent(const struct inotify_event *pEvt, InotifyWatch *pWatch) : m_uMask(0), m_uCookie(0) {
        if (pEvt != NULL) {
     ...
     */
    inline static bool IsType(uint32_t uValue, uint32_t uType) { return ((uValue & uType) != 0) && ((~uValue & uType) == 0); }
     ...
     * \param[out] rStr dumped event types
     */
    void dump_types(std::string &rStr) const;
...
class InotifyWatch {
  public:
    ...
    InotifyWatch(const std::string &rPath, int32_t uMask, bool fEnabled = true)
        : m_path(rPath), m_uMask(uMask), m_wd((int32_t)-1), m_fEnabled(fEnabled) {
        IN_LOCK_INIT
    }
...
class Inotify {
  public:
     ...
     * \throw InotifyException thrown if removing failed
     */
    inline void Remove(InotifyWatch &rWatch) noexcept(false) { Remove(&rWatch); }
     ...
     * \sa SetNonBlock()
     */
    void wait_for_events(bool fNoIntr = false) noexcept(false);
     ...
     */
    bool get_event(InotifyEvent *pEvt) noexcept(false);
     ...
     */
    bool GetEvent(InotifyEvent &rEvt) noexcept(false) { return get_event(&rEvt); }
     ...
     * \throw InotifyException thrown only in very anomalous cases
     */
    bool PeekEvent(InotifyEvent &rEvt) noexcept(false) { return PeekEvent(&rEvt); }
     ...
     * \throw InotifyException thrown if the given value cannot be acquired
     */
    inline static uint32_t GetMaxEvents() noexcept(false) { return GetCapability(IN_MAX_EVENTS); }
     ...
     *            infrastructure.
     */
    inline static void SetMaxEvents(uint32_t val) noexcept(false) { SetCapability(IN_MAX_EVENTS, val); }
     ...
     * \throw InotifyException thrown if the given value cannot be acquired
     */
    inline static uint32_t GetMaxInstances() noexcept(false) { return GetCapability(IN_MAX_INSTANCES); }
     ...
     *            infrastructure.
     */
    inline static void SetMaxInstances(uint32_t val) noexcept(false) { SetCapability(IN_MAX_INSTANCES, val); }
     ...
     * \throw InotifyException thrown if the given value cannot be acquired
     */
    inline static uint32_t GetMaxWatches() noexcept(false) { return GetCapability(IN_MAX_WATCHES); }
     ...
     *            infrastructure.
     */
    inline static void SetMaxWatches(uint32_t val) noexcept(false) { SetCapability(IN_MAX_WATCHES, val); }
